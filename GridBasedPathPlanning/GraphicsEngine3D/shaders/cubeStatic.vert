...
vec3 Colormap_old(in float val){
...
vec3 Colormap(in float val){
    float R = 2.335*val*val*val - 5.957*val*val + 3.516*val + 0.316;
    float G = 2.774*val*val*val - 5.416*val*val + 2.151*val + 0.623;
    float B = 2.172*val*val*val - 4.441*val*val + 2.178*val + 0.129;
    return clamp( vec3(R,G,B), 0.0, 1.0 );
}
...
void main()
{
    ...
    v_color         = vec4(Colormap(a_instance_index.z/float(shape.z) + a_instance_value*0.001), 1.0); // vec4(HUEtoRGB(0.66 * (a_instance_value)), 1.0);
