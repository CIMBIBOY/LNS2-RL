...
SUBFOLDER = 'WorkingParams' # Choose or create a folder inside Data/Processing
FOLDER = ROOTFOLDER + SUBFOLDER + '/'

def PathPlanning_TJPS_3D():
    ...
    for drone_idx in range(DRONES):
        ...
        # Save the grid_seq without any planned drones
        if drone_idx == 0:
            saveData_Numpy(FOLDER, grid_seq=grid_seq, grid_static=grid.grid_static)

        # Radarmap 3D
        #print("Start: ", start := (4,126,8)) # (2,63,6))
        #print("Goal: ",  goal := (126,58,8)) # (63,29,6))
        # Random terrain
        print("Start: ", start := grid.GetRandomFreeCell((2,46,17), r=3, max_attempts=50))
        print("Goal: ",  goal  := grid.GetRandomFreeCell((168,40,16), r=3, max_attempts=50))
