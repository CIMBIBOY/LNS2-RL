...
SUBFOLDER = 'multistep22' # Choose or create a folder inside Data/Processing
FOLDER = ROOTFOLDER + SUBFOLDER + '/'

START = (0,145,24)
GOAL = (195,40,24)
START_RADIUS = (10,15,5)
GOAL_RADIUS = (10,30,5)

np.random.seed(0)

def PathPlanning_3D():
    grid_static = generateRandomTerrain_Simplex2D(shape_0:=(200,150,40), scale=max(shape_0)/2.2, octaves=1, persistence=1.8, lacunarity=10, seed=3)   # Random generated terrain

    grid = GridMapEnv(grid_size=shape_0) # Create the Grid environment
    grid.SetStaticGrid(grid_static, convert_nonzero_to_inf=True)
    grid.addRandomDynamicObstacles(no_of_obs=20, d_range=(10,40), alpha = 0.5)
    for peak in getListOfHighestMountains(grid_static, n=5, proximity_divisor=2.5):
        grid.CreateObstacle(typ='dynamic', shape='Radar', data={'p':peak, 'a':(18,20), 'n':1.5})
...
if __name__ == '__main__':
    PathPlanning_3D()
    PathPlanning_PostProcess(FOLDER, interpolate_MinimumSnapTrajectory=True, obs_threshold_spline=0.51)
