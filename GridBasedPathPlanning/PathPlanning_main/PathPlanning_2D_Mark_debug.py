...
def PathPlanning_TJPS_2D():
    ...
    '''
    grid_seq = np.load('GridBasedPathPlanning/Data/BenchmarkMaps/grid_seq_1.npy')
    start = (62,48)
    goal =  (56,13)
    ...
    planner = TJPS(grid_seq, start, goal, max_wait=4)
    plan = planner.plan() if PLAN else None


    # PLOT
    app = GraphicsEngine2D(grid_seq, plan['path_extracted'], win_size=(1000,1000), FPS_animation=2)
    app.run()
