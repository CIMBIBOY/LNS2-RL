...
def dijkstra(grid:np.ndarray, start:Node_Dijkstra, goal:Node_Dijkstra, motions:list[Node_Dijkstra]):
    ...
    while OPEN:
        ...
        for new_node in getNeighbor_Dijkstra(node, grid, grid.shape):          
...
def getNeighbor_Dijkstra(node: Node_Dijkstra, grid:np.ndarray, shape: np.ndarray) -> list:
    neighbors = []
...
def extractPath_Dijkstra(closed_set, start, goal, motions):
    node = closed_set[closed_set.index(goal)] # goal
    path = [node]
    while node != start:
...
def g_Disjkstra(grid, node, move, new_node):
    return node.g + move.g * (1 + (grid[node.current] + grid[(new_node).current])/2) # average current and dest cells weights and mult with path len
...
if __name__ == '__main__':
    ...
    path = dijkstra(grid, start_node, goal_node, motions)

    for node in path: print(node)
    print("Last move: ", motions[path[-1].parent_motion_idx].current)