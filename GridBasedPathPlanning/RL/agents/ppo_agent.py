    ...
    def update(self, obs, actions, masks, logprobs, advantages, returns, values, init_lstm, dones):
        ...
        for epoch in range(self.args.update_epochs):
            ...
            for start in range(0, self.args.num_envs, envsperbatch):
                ...
                _, newlogprob, entropy, _ = self.model.get_action_and_value(
                    obs[mb_inds],
                ...
                # Flatten the values to match the expected shape
                values = values.view(-1)

                # Define the target as 1 for all values, since this is the ideal outcome
                target_values = torch.full_like(values, 1.0)

                # Calculate the value loss as the mean squared error between values and the target (1)
                v_loss = 0.5 * ((values - target_values) ** 2).mean()
                
                '''
                # Logging value loss details
                print(f"v_loss: {v_loss.item():.4f}")
                print(f"newvalue mean: {values.mean().item():.4f}, min: {values.min().item():.4f}, max: {values.max().item():.4f}")
    ...
    def learn(self):
        ...
        for update in range(1, num_updates + 1):
            ...
            for step in range(0, self.args.num_steps):
                ...
                with torch.no_grad():
                    action, logprob, entropy, next_lstm_state = self.model.get_action_and_value(
                        next_obs, next_lstm_state, next_done, action_masks[step]
                    ) # value
                    value = torch.zeros(self.args.num_envs).to(self.device)
                    for i in range(self.args.num_envs):
                        value[i] = self.env.envs[i].state_estimate
                    values[step] = value
                actions[step] = action
            ...
            with torch.no_grad():
                ...
                for i in range(self.args.num_envs):
                    next_value[i] = self.env.envs[i].state_estimate
                if self.args.gae:
            ...
            if update % (self.args.cmd_log * 10) == 0:
                # Save model weights
                self.save(f'GridBasedPathPlanning/RL/eval/weights/{self.run_name}.pth')
