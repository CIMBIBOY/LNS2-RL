...
from GridBasedPathPlanning.Plan.TJPS import TJPS_2D

def save_evaluation_image(init_arr, start, end, agent_path, tjps_path, episode, eval_folder='grid-based-path-planning/GridBasedPathPlanning/RL/eval/eval_images'):
    """
    ...
    # Plot optimal path
    tjps_path = np.array(tjps_path)
    plt.plot(tjps_path[:, 1], tjps_path[:, 2], 'blue', linestyle='-', linewidth=2, label='Optimal Path')
...
def evaluate_performance(env, args, num_episodes=100, agent = None, train_name = 'czm', eval_folder="GridBasedPathPlanning/RL/eval/eval_images", wandb=None):
    """
    ...
    for episode in range(num_episodes):
        start_cell = tuple(env.envs[0].reset_state[0][1:3])
        end_cell = tuple(env.envs[0].agents_paths[-1][1:3])

        assert start_cell != end_cell, "Start and end cells are the same"
        print(f" ---------- Episode {episode + 1}/{num_episodes} ---------- ")
        
        planner = TJPS_2D( env.envs[0].grid_seq, start_cell, end_cell, max_wait=1, log=0)
        plan = None
        while plan is None:
            plan = planner.plan()
        optimal_path = plan['path_extracted'].astype(np.int64)
      
        if optimal_path.all() == None:
        ...
        while not termination_flags:
            with torch.no_grad():
                action, _, _, lstm_state = agent.model.get_action_and_value(
        ...
        print(f"  Optimal path length: {optimal_path_length -1}")
        ...
        save_evaluation_image(env.envs[0].init_arr, start_cell, end_cell, path, optimal_path, episode, eval_folder)
